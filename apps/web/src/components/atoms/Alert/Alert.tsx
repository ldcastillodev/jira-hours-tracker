...
const variantStyles: Record<NonNullable<AlertProps['variant']>, string> = {
  inline: 'rounded-lg border border-mgs-red/30 bg-mgs-red/10 px-3 py-2 text-xs text-mgs-red-light',
  section:
...
export function Alert({ children, className, variant = 'inline' }: AlertProps) {
  return <div className={className ?? variantStyles[variant]}>{children}</div>;
}
